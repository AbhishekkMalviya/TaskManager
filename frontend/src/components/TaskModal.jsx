...
const TaskModal = ({ isOpen, onClose, taskToEdit, onSave, onLogout }) => {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    if (taskData.dueDate < today) {
      setError('Due Date can be in the past.');
      return;
    }
    setLoading(true)
    setError(null)
    try {
      ...
      console.error(err);
      setError(err.message || 'An unexpected error had occured!');
    }
    finally {
      setLoading(false);
    }
              ...
              {[{ val: 'Yes', label: 'Completed' }, { val: 'No', label: 'In Progress' }].map(({ val, label }) => (
                <label key={val} className=' flex items-center'>
                  <input type='radio' name='completed' value={val} checked={taskData.completed === val}
                    onChange={handleChange} className=' h-4 w-4 text-purple-600 focus-within:ring-purple-500 border-gray-300 rounded' />
