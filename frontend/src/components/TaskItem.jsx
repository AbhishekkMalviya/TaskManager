...
import { Calendar, CheckCircle2, Clock, MoreVertical, TicketPlus } from 'lucide-react'
import axios from 'axios'
import { format, isToday } from 'date-fns'
const API_BASE = 'http://localhost:4000/api/tasks'
import TaskModal from './TaskModal'

const TaskItem = ({ task, onRefresh, onLogout, showCompletedCheckbox }) => {
  ...
  //   try {
  //     const payload = (({ title, description, priority, dueDate, completed }))(updatedTask)
  ...
  setShowEditModal(false);
  onRefresh?.(); // This re-fetches the task list if needed
}
              ...
              <div className={TI_CLASSES.menuDropdown}>
                {MENU_OPTIONS.map(opt => (
                  <button key={opt.action} onClick={() => handleAction(opt.action)}
                    className=' w-full px-3 sm:px-4 y-2 text-left text-s space-x-2 sm:text-sm hover:bg-purple-100 flex items-center gap-2 transition-colors duration-200'>
                    {opt.icon} {opt.label}
                  </button>
                ))}
              </div>
