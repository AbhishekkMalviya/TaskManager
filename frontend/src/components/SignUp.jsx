...
const SignUp = ({ onSwitchMode }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ text: err.response?.data?.message || "An Error Occured, Please Try Again in Some-time.", type: "error" })
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className='max-w-md w-full bg-white shadow-lg border border-purple-100 rounded-xl p-8'>
      ...
      <p className='text-center text-sm text-gray-600 mt-6'>
        ...
        <button onClick={onSwitchMode} className='text-green-600 hover:text-green-700 hover:underline font-medium transition-colors'>
          Login
        </button>
      </p>
    </div>
